...
export function HabitDay({defaultCompleted = 0, amount = 0, date}: HabitDayprops){
    ...
    function handleCompletedChaged(defaultCompleted: number) {
    ...
    return (
        <Popover.Root>
            ...
            <Popover.Trigger 
                className={clsx('w-10 h-10 border rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-violet-900 focus:ring-offset-2 focus:ring-offset-background', {
            ...
            <Popover.Portal>
                <Popover.Content className='min-w-[320px] w-full p-6 rounded-2xl bg-zinc-900 flex flex-col'>
                    <span className='font-semibold text-zinc-400 leading-tight text-1xl'>{dayOfWeek}</span>
                    ...
                    <HabitsList
                        date={date}
                        onCompletedChenged={handleCompletedChaged}
                    />
